...
def parse_income_data(data: List[TransactionData], group_details) -> List[IncomeDataMonth]:
    result_list = []
    ...
    for transaction in data:
        if transaction["category"] in ["MONTLY_INCOME", "EXTRAORDINARY_INCOME"]:
...
def parse_expense_data(data: List[TransactionData], group_details: dict) -> List[ExpenseDataMonth]:
    result_list = []
    months = get_months(group_details["created_at"])
    for month in months:
        result_list.append({    
...
def parse_debt_data(data: List[TransactionData], group_details: dict) -> List[DebtDataMonth]:
    result_list = []
    months = get_months(group_details["created_at"])
    for month in months:
        result_list.append({
...
def parse_group_details(group_details: dict, parsed_data: ParsedData) -> dict:
    ...
    group_details["balance"] = (
        group_details["total_income"] + group_details["total_debt"] - group_details["total_expense"]
    )
